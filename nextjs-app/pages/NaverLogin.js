...
function NaverLogin(props) {
    ...
    useEffect(()=>{
        ...
        const { naver } = window;
        const NAVER_CLIENT_ID = 'xXGIgvZkDq8Wvx9GwnWP'; 
        const NAVER_CALLBACK_URL = "http://127.0.0.1:3001/MovieListComponent";
        const initializeNaverLogin = () => {
            ...
            naverLogin.getLoginStatus(async function (status) {
                if (status) {
                    ...
                    window.location.replace('/MovieListComponent');//상태 값을 읽어버려서 추가
                }
            })
        }
    ...
    function logout() {
        ...
        localStorage.clear();
        window.location.replace('/MovieListComponent'); //로그아웃시 기존 상태 값 제거하기 위해서 추가
    }
